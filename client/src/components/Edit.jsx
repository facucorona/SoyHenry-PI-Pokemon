...
function Edit() {
  ...
  let [newPokemonObject, setNewPokemonObject] = useState({
    ...thisPokemon,
    pokemonType: thisPokemon[0].pokemonType,
  });
  ...
  function handleChange(e) {
    ...
    if (e.target.name === "hp") {
      if (!regExpNumber.test(e.target.value) || !e.target.value > 0) {
        setHpAdvert(false);
      } else {
        setHpAdvert(true);
        setNewPokemonObject({
    ...
    if (e.target.name === "image") {
      if (!regExpUrl.test(e.target.value)) {
        setImageAdvert(false);
      } else {
        setImageAdvert(true);
        setNewPokemonObject({
          ...newPokemonObject,
